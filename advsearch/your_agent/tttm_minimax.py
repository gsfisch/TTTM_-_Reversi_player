...
def make_move(state: GameState) -> Tuple[int, int]:
    ...
    # Preencha a funcao utility com o valor de um estado terminal e
    # passe-a como funcao de avaliação para seu minimax_move
...
def utility(state, player: str) -> float:
    ...
    # Assume que player == "W"
    if not board.check_loser():
        return 0

    elif board.check_loser() == "W":
        answer = -1

    else:
        answer = 1

    # Corrige caso player == "B"
    if player == "B":
        answer = (-1) * answer
...
player = 'W'
gameState = GameState(myBoard, player)

print(utility(gameState, 'B'))
'''
